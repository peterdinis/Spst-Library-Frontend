...
export const authApi = {
	...
	health: async (): Promise<AuthResponse> => {
		const response = await fetch(
			`${env.VITE_BACKEND_IDENTITY_API}/auth/health`,
		);
	...
	register: async (registerData: RegisterDto): Promise<AuthResponse> => {
		const response = await fetch(
			`${env.VITE_BACKEND_IDENTITY_API}/auth/register`,
			{
				method: "POST",
				headers: {
	...
	login: async (loginData: LoginDto): Promise<UserResponseDto> => {
		const response = await fetch(
			`${env.VITE_BACKEND_IDENTITY_API}/auth/login`,
			{
				method: "POST",
				headers: {
					"Content-Type": "application/json",
	...
	logout: async (): Promise<AuthResponse> => {
		const response = await fetch(
			`${env.VITE_BACKEND_IDENTITY_API}/auth/logout`,
			{
				method: "POST",
				headers: {
					"Content-Type": "application/json",
	...
	getCurrentUser: async (): Promise<UserResponseDto> => {
		const response = await fetch(
			`${env.VITE_BACKEND_IDENTITY_API}/auth/current-user`,
			{
				method: "GET",
				headers: {
					"Content-Type": "application/json",
	...
	accessDenied: async (): Promise<AuthResponse> => {
		const response = await fetch(
			`${env.VITE_BACKEND_IDENTITY_API}/auth/access-denied`,
		);
...
export class AuthService {
	...
	async getCurrentUser() {
		return this.makeRequest<UserResponseDto>(
			"/auth/current-user",
			"GET",
			undefined,
			{
